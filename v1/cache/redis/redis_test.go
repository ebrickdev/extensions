...
func TestNewRedis(t *testing.T) {
	...
	// When
	st := NewRedis(client, store.WithExpiration(6*time.Second))

	// Then
	assert.IsType(t, new(RedisStore), st)
	assert.Equal(t, client, st.client)
	assert.Equal(t, &store.Options{Expiration: 6 * time.Second}, st.options)
}
...
func TestRedisSet(t *testing.T) {
	...
	// When
	err := st.Set(ctx, cacheKey, cacheValue, store.WithExpiration(5*time.Second))
...
func TestRedisSetWhenNoOptionsGiven(t *testing.T) {
	...
	store := NewRedis(client, store.WithExpiration(6*time.Second))
...
func TestRedisSetWithTags(t *testing.T) {
	...
	// When
	err := st.Set(ctx, cacheKey, cacheValue, store.WithTags([]string{"tag1"}))
...
func TestRedisInvalidate(t *testing.T) {
	...
	// When
	err := st.Invalidate(ctx, store.WithInvalidateTags([]string{"tag1"}))
